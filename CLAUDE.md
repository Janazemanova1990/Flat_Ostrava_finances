...
- **Mortgage card:** info tooltip (ⓘ) shows rate/term/payoff date; large payment row; on mobile P/I breakdown shows as 2 stacked rows below the amount. Payoff date formatted DD.MM.YYYY. `monthsElapsed` counts a new month from the payment day (derived from `startDate`'s day-of-month) rather than the 1st — so if start date is 2026-06-19, the counter ticks on the 19th of each month. Mortgage start date set to 2026-06-19 (first full payment due June 19; May 2026 was interest-only pre-mortgage fee).
